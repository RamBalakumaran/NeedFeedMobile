  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.ngoBox}>
        ...
        <Text style={styles.ngoName}>{item.requestedBy?.organizationName || item.requestedBy?.name || "Unknown NGO"}</Text>
        ...
        <View style={styles.acceptedBox}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('Chat', {
              requestId: item._id,
              name: item.requestedBy?.organizationName || item.requestedBy?.name || "NGO",
              status: item.status
            })}
          >
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#FFF', borderRadius: 15, padding: 20, marginBottom: 15, elevation: 3, shadowColor: '#000', shadowOpacity: 0.1 },
  ...
  acceptedBox: { marginTop: 15, alignItems: 'center', width: '100%' },
  acceptedText: { color: 'green', fontWeight: 'bold', marginBottom: 10 },
  chatBtn: { flexDirection: 'row', backgroundColor: COLORS.blue, padding: 12, borderRadius: 10, alignItems: 'center', width: '100%', justifyContent: 'center' }
});
