...
const RegisterScreen = ({ navigation }) => {
  ...
  const [coords, setCoords] = useState(null); // 2. State for GPS
  ...
  // 3. Fetch Location on Mount
  useEffect(() => {
    (async () => {
      let { status } = await Location.requestForegroundPermissionsAsync();
      if (status !== 'granted') {
        alert('Permission to access location was denied. GPS features will not work.');
        return;
      }
  ...
  const handleRegister = async () => {
    ...
    // 4. Append Latitude/Longitude to Payload
    const payload = {
      ...form, 
      role,
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{flex:1}}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.titleSection}>
            ...
            {!coords && <Text style={{color: 'orange', fontSize: 10, marginTop: 5}}>⌛ Fetching GPS Location...</Text>}
          </View>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: COLORS.bg, paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight + 10 : 10 },
